...
import { getDetailUserRequest } from "../../utils/api/UserApi";
import {
  CheckoutRequest,
  GenerateQris,
  GenerateVa,
...
export default function Detail() {
  ...
  const { listTicket, merchantDetail } = useGlobalContext();
  ...
  const handleSubmit = (value, actions) => {
    CheckoutRequest({
      ...
      .then((res) => {
        // console.log(res);
        const sales_id = res.data.data.sales_id;
        if (value.payment === "qris") {
          router.push(
            {
              pathname: "/payment/qris",
              query: { sales_id: sales_id, email: value.email },
            },
            "/payment/qris"
          );
        }
      ...
      <Box>
        <Stack spacing={2} bgColor="white" borderRadius="10px" p="10">
          ...
          <Box display="flex" alignItems="center" justifyContent="center">
            <Image
              src={merchantDetail?.img}
              alt={merchantDetail?.name}
              width="100px"
              height="70px"
              objectFit="cover"
              style={{
              ...
              if (item.adult > 0) {
                return (
                  <QtyTiket
                    ...
                    type={item.adult_label}
                    price={IDRConvert.format(item.adult * item.adult_price)}
                    qty={item.adult}
                  />
                );
              }
              if (item.child > 0) {
                return (
                  <QtyTiket
                    key={key}
                    name={item?.ticket_name}
                    type={item.child_label}
                    price={IDRConvert.format(item.child * item.child_label)}
                    qty={item.child}
                  />
                );
              }
